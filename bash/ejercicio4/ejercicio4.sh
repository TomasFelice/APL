...
# Normalize paths (preserve spaces)
...
# Determinar rama principal remota (preferir origin/main, fallback origin/master)
UPSTREAM=""
if git rev-parse --verify origin/main >/dev/null 2>&1; then
  ...
  # intentar detectar HEAD remoto
  head_ref=$(git remote show origin 2>/dev/null | awk -F': ' '/HEAD branch/ {print $2}')
  if [ -n "$head_ref" ] && git rev-parse --verify "origin/$head_ref" >/dev/null 2>&1; then
    UPSTREAM="origin/$head_ref"
  else
    friendly_exit "No se pudo determinar la rama principal remota (origin/main o origin/master)."
  fi
fi
...
# Hacer un fetch inicial y tomar el commit actual
if ! git fetch origin --quiet 2>/dev/null; then
  friendly_exit "No se pudo hacer 'git fetch origin' en '$REPO'. Verifique red/remote."
fi
...
scan_diff() {
  ...
  for f in "${files[@]}"; do
    ...
    for patt in "${patterns_literal[@]}"; do
      # grep: 0 match -> exit 1, 2->error
      if grep -nH --line-number -F -- "$patt" -- "$full" 2>/dev/null | while IFS= read -r matchline; do
        # registrar alerta por cada linea encontrada
        log_alert "Alerta: patrón '$patt' encontrado en el archivo '$f' -> ${matchline#*:}"
      done; then
        true
      else
        # 1 = sin coincidencias; 2 = error
        grep_exit=$?
    ...
    for rp in "${patterns_regex[@]}"; do
      if grep -nH --line-number -E -- "$rp" -- "$full" 2>/dev/null | while IFS= read -r matchline; do
...
while true; do
  ...
  if ! git fetch origin --quiet 2>/dev/null; then
    log_alert "Error: 'git fetch origin' falló en '$REPO'. Se intentará en la siguiente iteración."
    continue
  fi
