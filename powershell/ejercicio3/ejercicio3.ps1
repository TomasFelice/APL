...
function Invoke-LogFileProcessing {
    param(
        [Parameter(Mandatory=$true)]
            ...
            try {
                ...
                foreach ($line in $content) {
                    ...
                    foreach ($word in $wordsArray) {
                        ...
                        $regexMatches = [regex]::Matches($lowerLine, [regex]::Escape($lowerWord))
                        $wordCounts[$word] += $regexMatches.Count
                    }
                }
            } catch {
...
try {
    ...
    try {
        ...
        $success = Invoke-LogFileProcessing -DirectoryPath $validatedDirectory -WordList $cleanWords
        if (-not $success) {
